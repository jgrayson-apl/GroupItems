...
], function (applicationConfig, boilerplateSettings,
             kernel, lang, Deferred, esriConfig,
             Promise, promiseList, IdentityManager, OAuthInfo,
  ...
  return Promise.createSubclass({
    ...
    _queryGroupInfo: function () {
      ...
      if(!this.settings.group.fetchInfo || !this.config.group) {
        ...
        var params = new PortalQueryParams({
          query: lang.replace("(id:{group})", this.config)
        });
    ...
    _queryPortal: function () {
      ...
      if(!this.settings.portal.fetch) {
        ...
        this.portal = new Portal({ authMode: this.settings.portal.authMode });
        this.portal.load().then(function (response) {
